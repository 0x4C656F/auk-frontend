<script lang="ts">
	...
	import { enumToString, programIterator } from '$root/lib/entities';
...
>
	...
	<section class="form-control w-full max-w-xs">
		<select required name="program" class="select select-bordered w-full max-w-xs">
			<option disabled selected>Program</option>
			{#each programIterator() as program}
				<option value={program}>{enumToString(program)}</option>
			{/each}
		</select>
		{#if form?.target === 'program'}
			<p class="text-error">{form.message}</p>
		{/if}
	</section>
