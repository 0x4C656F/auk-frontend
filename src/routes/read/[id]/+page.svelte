...
<main class="mt-16 max-w-screen-md mx-auto px-4 py-8 font-sans-serif">
	<article class="prose lg:prose-xl flex flex-col gap-4">
		...
		<div class="gap-0 space-y-4">
			...
			<h2 class="text-xl text-text-muted">{post.subheading}</h2>
		</div>

		<div class="flex items-center gap-4 mb-8">
			<div class="size-12 rounded-full bg-secondary"></div>
			<div>
		...
		<div class="border-t border-text-muted/20 pt-8">
			<h3 class="text-xl font-semibold mb-4">About the author</h3>
			<div class="flex items-center space-x-4">
				<div class="size-16 rounded-full bg-secondary"></div>
				<div>
					<p class="font-semibold">{post.author.fullname}</p>
					<p class="text-sm text-gray-500">{'Here will be author`s biography'}</p>
				</div>
			</div>
		</div>
	</article>
